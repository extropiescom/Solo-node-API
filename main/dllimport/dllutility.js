const { DLLDEVINFO, DLLPSTEP, DLLPSTATUS, DLLCOINTYPE, DLLCONST, DLLRET } = require("./dllconst");
const sprintf = require("sprintf-js");

//util functions
const DLLUTIL = {};

DLLUTIL.ewallet_ret2string = res => {
    var strResult = "";
	...
	case DLLRET.PAEW_RET_DEV_PIN_VERIFY_FAIL:
		...
		break;
	case DLLRET.PAEW_RET_DEV_CHECKDATA_FAIL:
		strResult = "device check data failed";
		break;
	case DLLRET.PAEW_RET_DEV_DEV_OPERATING:
		strResult = "device is on operating";
		break;
	case DLLRET.PAEW_RET_DEV_PIN_UNINIT:
		strResult = "PIN not inited";
		break;
	case DLLRET.PAEW_RET_DEV_BUSY:
		strResult = "device busy";
		break;
	case DLLRET.PAEW_RET_DEV_ALREADY_AVAILABLE:
		strResult = "device already available, don't need to abort";
		break;
	case DLLRET.PAEW_RET_DEV_DATA_NOT_FOUND:
		strResult = "device data not found";
		break;
	case DLLRET.PAEW_RET_DEV_SENSOR_ERROR:
		strResult = "device sensor error";
		break;
	case DLLRET.PAEW_RET_DEV_STORAGE_ERROR:
		strResult = "device storage error";
		break;
	case DLLRET.PAEW_RET_DEV_STORAGE_FULL:
		strResult = "device storage full";
		break;
	case DLLRET.PAEW_RET_DEV_FP_COMMON_ERROR:
		strResult = "finger print command error";
		break;
	case DLLRET.PAEW_RET_DEV_FP_REDUNDANT:
		strResult = "redundant fingerprint";
		break;
	case DLLRET.PAEW_RET_DEV_FP_GOOG_FINGER:
		strResult = "good fingerprint";
		break;
	case DLLRET.PAEW_RET_DEV_FP_NO_FINGER:
		strResult = "not fingerprint";
		break;
	case DLLRET.PAEW_RET_DEV_FP_NOT_FULL_FINGER:
		strResult = "not full fingerprint";
		break;
	case DLLRET.PAEW_RET_DEV_FP_BAD_IMAGE:
		strResult = "bad fingerprint image";
		break;
	case DLLRET.PAEW_RET_DEV_LOW_POWER:
		strResult = "device low power";
		break;
	case DLLRET.PAEW_RET_DEV_TYPE_INVALID:
		strResult = "device type invalid";
		break;
	case DLLRET.PAEW_RET_NO_VERIFY_COUNT:
		strResult = "no verify count";
		break;
	case DLLRET.PAEW_RET_AUTH_CANCEL:
